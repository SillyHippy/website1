...
jQuery( document ).ready( function( $ ) {
  ...
  $('.wprmenu_icon_par').on('click', function() {
    var t = $(this), child_ul = t.parent('li').find('ul.sub-menu').first();
  ...
  function close_sub_uls() {
    menu.find('ul.sub-menu').each(function() {
      var ul = $(this), icon = ul.parent('li').find('.wprmenu_icon_par'), li = ul.parent('li');
   ...
   * Redirects hash links to the homepage first if not on `/`
   */
  $('#wprmenu_menu_ul a').click(function(event) {
    ...
    if (targetHref.startsWith("#")) {
      event.preventDefault();
      window.location.href = "/" + targetHref;  // Ensures navigation to homepage first
    }
