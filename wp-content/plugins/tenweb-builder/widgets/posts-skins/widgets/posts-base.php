...
use Tenweb_Builder\ElementorPro\Modules\LoopBuilder\Module as LoopBuilderModule;
...
abstract class Posts_Base extends Widget_Base {
	...
	public function register_load_more_message_style_controls() {
		$this->add_control(
			...
			[
				'label' => esc_html__( 'No More Posts Message', 'tenweb-builder' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
				'condition' => [
					'pagination_type' => 'load_more_on_click',
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
                    '{{WRAPPER}} .e-load-more-message' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Spinner Color', 'tenweb-builder' ),
				'type' => Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .e-load-more-spinner' => 'color: {{VALUE}};',
				],
				'separator' => 'before',
				'condition' => [
	...
	public function register_pagination_section_controls() {
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Page Limit', 'tenweb-builder' ),
				'default' => '5',
				'condition' => [
					'pagination_type!' => [
						'load_more_on_click',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Shorten', 'tenweb-builder' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Previous Label', 'tenweb-builder' ),
				'dynamic' => [
					'active' => true,
				],
				'default' => esc_html__( '&laquo; Previous', 'tenweb-builder' ),
				'condition' => [
					'pagination_type' => [
						'prev_next',
						'numbers_and_prev_next',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Next Label', 'tenweb-builder' ),
				'default' => esc_html__( 'Next &raquo;', 'tenweb-builder' ),
				'condition' => [
					'pagination_type' => [
						'prev_next',
						'numbers_and_prev_next',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Individual Pagination', 'tenweb-builder' ),
				'type' => Controls_Manager::SWITCHER,
		...
		$this->add_control(
			...
			[
				...
				'raw' => esc_html__( 'For multiple Posts Widgets on the same page, toggle this on to control the pagination for each individually. Note: It affects the page\'s URL structure.', 'tenweb-builder' ),
		...
		$this->add_control(
			'load_more_spinner',
			[
				'label' => esc_html__( 'Spinner', 'tenweb-builder' ),
		...
		$this->register_button_content_controls( [
			'button_text' => esc_html__( 'Load More', 'tenweb-builder' ),
			'control_label_name' => esc_html__( 'Button Text', 'tenweb-builder' ),
			'section_condition' => [
				'pagination_type' => 'load_more_on_click',
			],
			'exclude_inline_options' => [ 'svg' ],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .e-load-more-message' => 'text-align: {{VALUE}};',
				],
				'condition' => [
					'pagination_type' => [
						'load_more_on_click',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Custom Messages', 'tenweb-builder' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'condition' => [
					'pagination_type' => [
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_html__( 'No more posts to show', 'tenweb-builder' ),
				'condition' => [
					'pagination_type' => [
						'load_more_on_click',
						'load_more_infinite_scroll',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Colors', 'tenweb-builder' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
	...
	public function get_current_page() {
		...
		return max(
			...
			get_query_var( 'paged' ),
			get_query_var( 'page' ),
			Utils::_unstable_get_super_global_value( $_GET, 'e-page-' . $this->get_id() ) //phpcs:ignore WordPress.Security.NonceVerification.Recommended
		);
	}
	...
	public function get_wp_link_page( $i ) {
		...
		if ( $this->is_rest_request() ) {
			...
			$post_id = url_to_postid( $link_unescaped ); //phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.url_to_postid_url_to_postid
	...
	protected function get_wp_link_page_url_for_preview( $post, $query_args, $url ) {
		...
		$query_args['preview_id'] = Utils::_unstable_get_super_global_value( $_GET, 'preview_id' ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$query_args['preview_nonce'] = Utils::_unstable_get_super_global_value( $_GET, 'preview_nonce' ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	protected function get_pagination_type_options() {
		return [
			...
			'numbers' => esc_html__( 'Numbers', 'tenweb-builder' ),
			'prev_next' => esc_html__( 'Previous/Next', 'tenweb-builder' ),
			'numbers_and_prev_next' => esc_html__( 'Numbers', 'tenweb-builder' ) . ' + ' . esc_html__( 'Previous/Next', 'tenweb-builder' ),
			self::LOAD_MORE_ON_CLICK => esc_html__( 'Load on Click', 'tenweb-builder' ),
			self::LOAD_MORE_INFINITE_SCROLL => esc_html__( 'Infinite Scroll', 'tenweb-builder' ),
		];
	}
