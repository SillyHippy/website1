...
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Module as ThemeBuilder;
...
trait Skin_Content_Base {
	...
	public function get_title() {
		return esc_html__( 'Full Content', 'tenweb-builder' );
	}
	...
	public function register_thumbnail_controls() {
		$this->add_control(
			'thumbnail',
			[
				'label' => esc_html__( 'Show Thumbnail', 'tenweb-builder' ),
	...
	public function render_post_content( $with_wrapper = false, $with_css = true ) {
		...
		if ( \Elementor\Plugin::instance()->preview->is_preview_mode( $post->ID ) ) {
			...
			if ( empty( $content ) ) {
				...
				wp_link_pages( [
					'before' => '<div class="page-links elementor-page-links"><span class="page-links-title elementor-page-links-title">' . esc_html__( 'Pages:', 'tenweb-builder' ) . '</span>',
					'after' => '</div>',
					'link_before' => '<span>',
					'link_after' => '</span>',
					'pagelink' => '<span class="screen-reader-text">' . esc_html__( 'Page', 'tenweb-builder' ) . ' </span>%',
