...
class Products_Renderer extends \WC_Shortcode_Products {
	...
	private function set_terms_query_args( &$query_args ) {
        ...
        if ( ! empty( $this->settings[ $this->settings_key_prefix . 'include_term_ids' ] ) || ! empty( $this->settings[ $this->settings_key_prefix . 'include_term_ids_tags' ] ) ) {
            $terms = [];
            ...
            if ( ! empty( $this->settings[ $this->settings_key_prefix . 'include_term_ids_tags' ] ) ) {
                foreach ($this->settings[$this->settings_key_prefix . 'include_term_ids_tags'] as $id) {
                    $term_data = get_term_by('term_taxonomy_id', $id);
                    $taxonomy = $term_data->taxonomy;
                    $terms[$taxonomy][] = $id;
                }
	...
	protected function set_exclude_query_args( &$query_args ) {
		if ( empty( $this->settings[ $this->settings_key_prefix . 'exclude' ] ) ) {
            $this->settings[ $this->settings_key_prefix . 'exclude' ] = [ // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude
                'terms',
                'authors',
            ];
		} else {
            $this->settings[ $this->settings_key_prefix . 'exclude' ] = array_merge([ // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude
                'terms',
                'authors',
            ],$this->settings[ $this->settings_key_prefix . 'exclude' ]);
        }
		$post__not_in = [];
		...
		$query_args['post__not_in'] = empty( $query_args['post__not_in'] ) ? $post__not_in : array_merge( $query_args['post__not_in'], $post__not_in ); // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn
		 ...
		 */
		if ( in_array( $this->settings[ $this->settings_key_prefix . 'post_type' ], [ 'sale', 'related_products', 'upsells', 'cross_sells' ], true ) ) {
			$query_args['post__in'] = array_diff( $query_args['post__in'], $query_args['post__not_in'] ); // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn
		}

        $terms = [];
		if ( in_array( 'terms', $this->settings[ $this->settings_key_prefix . 'exclude' ], true ) && ( ! empty( $this->settings[ $this->settings_key_prefix . 'exclude_term_ids' ] ) || ! empty( $this->settings[ $this->settings_key_prefix . 'exclude_term_ids_tags' ] ) ) ) {
            if (! empty( $this->settings[ $this->settings_key_prefix . 'exclude_term_ids' ] )) {
            ...
            if (! empty( $this->settings[ $this->settings_key_prefix . 'exclude_term_ids_tags' ] )) {
                foreach ($this->settings[$this->settings_key_prefix . 'exclude_term_ids_tags'] as $to_exclude) {
                    $term_data = get_term_by('term_taxonomy_id', $to_exclude);
                    $terms[$term_data->taxonomy][] = $to_exclude;
                }
            }
